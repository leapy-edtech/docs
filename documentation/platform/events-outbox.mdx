...
<Warning>
**Estado atual: o dispatcher é um STUB.** A escrita no outbox (insert na transação) já
funciona, mas o cron `dispatch-outbox-events` (`leapy/apps/inngest/src/functions/dispatch-outbox-events.ts`)
está registrado a cada 1 min apenas como marcador — **ainda não lê o outbox nem chama
`inngest.send()`**. Ou seja, eventos enfileirados hoje **não são despachados** até o
dispatcher real ser implementado (`repos.eventsOutbox.findPending` → `inngest.send` →
`markDispatched`). As seções abaixo descrevem o comportamento **projetado**.
</Warning>
