---
title: "Operações"
description: "Guia técnico de operações do sistema de Pulsos"
---

## Visão Geral

Este documento cobre operações técnicas do sistema de Pulsos, incluindo monitoramento de jobs, troubleshooting e procedimentos de manutenção.

<Note>
  Para guias práticos de uso diário, consulte: - [Guia de Operações
  CS/Ops](/guides/backoffice/pulsos-operacoes) -
...
<CardGroup cols={3}>
  <Card title="Criação" icon="plus">
    • createPulsosJovens • createPulsosLiderancas • processaPulsos
  </Card>
  <Card title="Status" icon="rotate">
    • updateStatusParaPendentes (4x) • updateStatusParaVencidos (2x)
  </Card>
  <Card title="Notificações" icon="envelope">
    • sendLembretes (3x) • sendLiberacoes (3x) • sendLembretePulsosRhToRh
  </Card>
</CardGroup>

### Métricas Críticas por Job

| Job                  | Success Rate | Latency Média | Alerta se      |
| -------------------- | ------------ | ------------- | -------------- |
...
- ⚠️ **Success Rate < 95%** em qualquer job de criação
- ⚠️ **Latency > 60s** em job de criação
- 🚨 **Falha total** em job de atualização de status
- 🚨 **> 10% bounce rate** em notificações
...
| Tipo            | Causa          | Ação                          |
| --------------- | -------------- | ----------------------------- |
| **Hard Bounce** | Email inválido | Corrigir email do talento     |
...
-- 3. Busca em texto (sem índice de texto completo)
...
2. Adicionar índices em campos frequentemente filtrados
3. Limitar resultados (paginação)
...
**Quando usar**: Bug foi corrigido e é necessário reprocessar pulsos afetados.
...
<Warning>Sempre teste com 1-2 pulsos antes de reprocessar em lote.</Warning>
...
**Automação**: Views são atualizadas automaticamente a cada 1 hora.
...
**Frequência**: Diário (automático via Supabase)

**Retenção**: 7 dias

**Localização**: Supabase Dashboard > Settings > Backups
...
2. Acessar backup mais recente antes da deleção
...
<Warning>
  Recovery manual deve ser último recurso. Sempre valide integridade dos dados
  após restore.
</Warning>
...
| ------------ | -------------------------- | -------------------------------- |
| **Inngest**  | app.inngest.com            | Execução de jobs, eventos, erros |
| **Resend**   | resend.com/emails          | Status de envio de emails        |
| **Supabase** | app.supabase.com           | Queries, slow queries, conexões  |
| **Directus** | backoffice.leapy.com/admin | Atividades de usuários           |
...
### Runbook 1: Pulsos Não Criados

**Sintoma**: É dia de criação mas nenhum pulso foi criado.
...
3. Verificar se há jovens ativos:
   ```sql
   SELECT COUNT(*) FROM talents WHERE status = 'active';
   ```

**Solução**: Ver [Troubleshooting completo](/guides/backoffice/pulsos-troubleshooting)

---

### Runbook 2: Email Não Enviado

**Sintoma**: Pulso criado mas talento não recebeu email.
...
4. Verificar se email do talento está correto (Directus)

**Solução**:

- Se bounced: Corrigir email
- Se não disparado: Reenviar via Inngest
...
2. Ver qual step está travado
3. Ver logs do step

**Solução**:

1. Cancelar job manualmente (botão "Cancel")
...
| ------------------------- | ---------------- | -------------------- |
| **Criação de pulso**      | < 1min por pulso | Latency do job       |
| **Envio de email**        | < 30s            | Resend delivery time |
| **Atualização de status** | < 5s             | Database query time  |
| **Cálculo de métricas**   | < 10s            | View refresh time    |

### Performance Targets

| Métrica                 | Target | Atual (Baseline) |
| ----------------------- | ------ | ---------------- |
...
<CardGroup cols={2}>
  ...
  >
    Detalhes técnicos dos jobs
  </Card>
  <Card
    title="Manual Técnico"
    icon="book"
    href="/guides/backoffice/pulsos-manual-tecnico"
  >
