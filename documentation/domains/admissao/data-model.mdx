...
| `inicio_ferias` | date | Data de início do período de férias (override por admissão individual) |
| `fim_ferias` | date | Data de término do período de férias |
| `periodo_ferias` | string | Representação textual derivada automaticamente ("DD/MM/YYYY a DD/MM/YYYY") |
...
| `inicio_ferias` | date | Período padrão de férias da janela (pode ser sobrescrito por admissão individual) |
| `fim_ferias` | date | Fim do período padrão de férias |
