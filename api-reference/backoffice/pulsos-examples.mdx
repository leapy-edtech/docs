...
description: "Casos de uso práticos com código completo para a API de Pulsos"
---

## Visão Geral

Exemplos práticos de integração com a API de Pulsos do Directus. Todos os exemplos assumem que você já possui um token de autenticação válido.

<Tip>
  Para obter o token de autenticação, consulte a [documentação do Backoffice](/api-reference/backoffice/index#autenticacao).
</Tip>
...
Fluxo completo: criar pulso para um jovem, agendar para daqui 3 dias e enviar notificação.
...
setTimeout(async () => {
  ...
  // 3. Disparar evento de notificação (via Inngest)
...
# Em produção, isso seria feito por um job agendado
requests.patch(
...
## Exemplo 2: Buscar Pulsos Pendentes por Área

Buscar todos os pulsos pendentes de jovens de uma área específica com seus dados completos.

<CodeGroup>

```typescript TypeScript
// Buscar pulsos pendentes da área de TI
...
Atualizar múltiplos pulsos vencidos para "pendente", estendendo o prazo.
...
const pulsosVencidos = await directus.request(
  readItems('pulsos_jovens', {
    filter: {
      _and: [
        { status: { _eq: 'vencido' } },
        { data_vencimento: { _gte: '$NOW(-7d)' } } // Vencidos há menos de 7 dias
      ]
    }
  })
);
...
Exportar todos os dados de um pulso específico, incluindo respostas de todos os jovens.
...
with open('pulso3_respostas.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=['Nome', 'Área', 'NPS Leapy', 'NPS Empresa', 'Feedback', 'Data'])
    writer.writeheader()
...
if (parseFloat(metricas.taxaResposta) < 50) {
  console.log('⚠️  Taxa de resposta abaixo de 50%! Considere enviar lembretes.');
}
```
...
if float(metricas['taxa_resposta'].rstrip('%')) < 50:
    print('⚠️  Taxa de resposta abaixo de 50%! Considere enviar lembretes.')
```
...
## Exemplo 6: Configurar Agendamento Automático
...
print('Agendamento trimestral configurado ✓')
```
